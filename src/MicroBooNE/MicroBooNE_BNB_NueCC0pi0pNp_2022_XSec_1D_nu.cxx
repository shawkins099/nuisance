...
#include "MicroBooNE_BNB_NueCC0pi0pNp_2022_XSec_1D_nu.h"

#include "TH1D.h"
#include "TH2D.h"

//********************************************************************
MicroBooNE_BNB_NueCC0pi0pNp_2022_XSec_1D_nu::MicroBooNE_BNB_NueCC0pi0pNp_2022_XSec_1D_nu(nuiskey samplekey) {
  //********************************************************************
  ...
  if (!name.compare("MicroBooNE_BNB_NueCC0pi0pNp_2022_XSec_1DElecCosTheta_nu")) {
    fDist = kElecCosTheta;
  ...
  else if (!name.compare("MicroBooNE_BNB_NueCC0pi0pNp_2022_XSec_1DElecEnergy_nu")) {
    fDist = kElecEnergy;
  ...
  else if (!name.compare("MicroBooNE_BNB_NueCC0pi0pNp_2022_XSec_1DProtonCosTheta_nu")) {
  ...
  else if (!name.compare("MicroBooNE_BNB_NueCC0pi0pNp_2022_XSec_1DProtonKE_nu")) {
    fDist = kProtonKE;
    objSuffix = "ProtonKE";
...
bool MicroBooNE_BNB_NueCC0pi0pNp_2022_XSec_1D_nu::isNpSignal(FitEvent *event) {
  // Check CC nue
...
bool MicroBooNE_BNB_NueCC0pi0pNp_2022_XSec_1D_nu::is0pSignal(FitEvent *event) {
  // Check CC nue
  if (!SignalDef::isCCINC(event, 12, EnuMin, EnuMax)) return false;
...
bool MicroBooNE_BNB_NueCC0pi0pNp_2022_XSec_1D_nu::isSignal(FitEvent *event) {
  if (is0p){
...
void MicroBooNE_BNB_NueCC0pi0pNp_2022_XSec_1D_nu::FillEventVariables(FitEvent *event) {  
  ...
  // if Np event, grab highest momentum proton vector
...
void MicroBooNE_BNB_NueCC0pi0pNp_2022_XSec_1D_nu::ConvertEventRates() {
  // standard conversion
